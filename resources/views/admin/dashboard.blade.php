...
@section('admin-content')
    <div class="grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
        <div class="rounded-[1.75rem] border border-white/10 bg-white/5 p-6">
            ...
            <h2 class="mt-3 font-display text-4xl text-white">Dashboard admin untuk blog Tirta.id</h2>
            <p class="mt-4 max-w-2xl text-sm leading-7 text-slate-300">
                Di sini admin mengelola konten. Query baca dipakai untuk daftar, preview, dan landing page. Query tulis dipakai saat create, update, dan delete post.
            </p>
            ...
            <div class="mt-6 flex flex-wrap gap-3">
                ...
                <a href="{{ route('home') }}" class="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-semibold text-white">Lihat landing publik</a>
            </div>

            <div class="mt-8 grid gap-4 sm:grid-cols-3">
                <div class="rounded-3xl border border-white/10 bg-white/5 p-5">
                    <p class="text-sm text-slate-400">Total post</p>
        ...
        <div class="rounded-[1.75rem] border border-amber-300/20 bg-amber-300/10 p-6 text-amber-50">
            <p class="text-xs uppercase tracking-[0.35em] text-amber-200">Read / write proof</p>
            <div class="mt-4 space-y-4 text-sm leading-7">
                ...
                <div class="rounded-2xl bg-slate-950/40 p-4">
                    <p class="font-semibold text-white">PGPool host</p>
                    <p class="mt-1 text-slate-200">{{ $dbHost }}:{{ $dbPort }}</p>
                </div>
                <div class="rounded-2xl bg-slate-950/40 p-4">
                    <p class="font-semibold text-white">Mode</p>
                    <p class="mt-1 text-slate-200">Single endpoint, PGPool II yang ngatur routing read/write.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mt-8 rounded-[1.75rem] border border-white/10 bg-white/5 p-6">
        <div class="flex items-center justify-between gap-4">
            <div>
                <p class="text-xs uppercase tracking-[0.35em] text-slate-400">Recent content</p>
        ...
        <div class="mt-6 space-y-3">
            ...
            @empty
                <div class="rounded-2xl border border-dashed border-white/10 bg-white/5 p-6 text-sm text-slate-300">
                    Belum ada posting. Buat artikel pertama untuk mengisi landing page.
                </div>
            @endforelse
        </div>
    </div>
