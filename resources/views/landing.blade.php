    ...
    <section class="grid gap-8 py-10 lg:grid-cols-[1.15fr_.85fr] lg:items-center lg:py-16">
        <div class="space-y-6">
            <div class="inline-flex items-center gap-2 rounded-full border border-slate-900/10 bg-white/70 px-4 py-2 text-sm font-medium text-slate-700 shadow-sm backdrop-blur">
                ...
                Landing publik untuk pembaca
            </div>

            <div class="space-y-4">
                <h1 class="font-display max-w-3xl text-5xl leading-none tracking-tight text-slate-950 md:text-7xl">
                    Berita dan blog yang terasa seperti newsroom kecil, bukan template biasa.
                </h1>
                <p class="max-w-2xl text-lg leading-8 text-slate-700">
                    Pengunjung langsung masuk ke landing page. Admin login terpisah, lalu masuk ke dashboard untuk CRUD posting berita/blog. Aplikasi cukup konek ke satu endpoint PGPool II di port 9999.
                </p>
            </div>
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('login') }}" class="rounded-full border border-slate-900/10 bg-white/80 px-6 py-3 text-sm font-semibold text-slate-900 backdrop-blur">Masuk admin</a>
            </div>

            <div class="grid gap-4 sm:grid-cols-3">
                <div class="rounded-3xl border border-slate-900/10 bg-white/70 p-5 shadow-sm backdrop-blur">
                    <p class="text-sm text-slate-500">Publikasi aktif</p>
                    <p class="mt-2 font-display text-3xl font-semibold">{{ $publishedCount }}</p>
                </div>
                <div class="rounded-3xl border border-slate-900/10 bg-white/70 p-5 shadow-sm backdrop-blur">
                    <p class="text-sm text-slate-500">Akses guest</p>
                    <p class="mt-2 font-display text-3xl font-semibold">Open</p>
                </div>
                <div class="rounded-3xl border border-slate-900/10 bg-white/70 p-5 shadow-sm backdrop-blur">
                    <p class="text-sm text-slate-500">Admin flow</p>
                    <p class="mt-2 font-display text-3xl font-semibold">CRUD</p>
                </div>
            </div>
        </div>
        ...
        <div class="relative">
            ...
            <div class="rounded-[2rem] border border-slate-900/10 bg-slate-950 p-6 text-slate-100 shadow-2xl shadow-slate-950/20">
                <div class="flex items-center justify-between border-b border-white/10 pb-4">
                    <div>
                        <p class="text-xs uppercase tracking-[0.35em] text-slate-400">Featured stack</p>
                ...
                <div class="mt-6 rounded-2xl bg-amber-300 p-4 text-slate-950">
                    <p class="text-xs uppercase tracking-[0.35em] text-slate-700">Read / write split</p>
                    <p class="mt-2 text-sm leading-6 text-slate-900">
                        Listing artikel membaca data, sedangkan create/update/delete dari dashboard menulis ke backend write.
                    </p>
                </div>
            </div>
        </div>
    ...
    <section id="latest" class="py-8">
        <div class="mb-6 flex items-end justify-between gap-4">
            ...
            <p class="max-w-xl text-sm leading-6 text-slate-600">
                Landing ini jadi pintu masuk pembaca, sedangkan admin kerja di dashboard terpisah supaya alur baca dan tulis gampang dibedakan.
            </p>
        </div>
        ...
        <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            ...
            @empty
                <div class="rounded-[1.75rem] border border-dashed border-slate-300 bg-white/70 p-8 text-sm text-slate-600 md:col-span-2 xl:col-span-3">
                    Belum ada artikel publik. Masuk ke admin untuk membuat posting pertama.
                </div>
            @endforelse
        </div>
    </section>
